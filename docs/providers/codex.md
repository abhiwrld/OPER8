...
This guide is for people who want to use more than one Codex account in OPER8.
...
In OPER8 Settings, name it something obvious:
...
In OPER8 Settings, add another Codex provider:
...
OPER8 shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, OPER8 treats it
