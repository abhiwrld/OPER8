...
This guide is for people who want to use more than one Claude setup in OPER8.
...
In OPER8 Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means OPER8 uses your normal home directory.
...
Then add another Claude provider in OPER8:
...
OPER8 only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so OPER8 keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in OPER8 Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. OPER8 stores the value as a server secret and does not
...
OPER8 does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in OPER8.
